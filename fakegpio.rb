...
class FakeGPIO
  ...
  def export; @redis.mset(rkey(:value),"\0",rkey(:direction),:in,rkey(:edge),:none,rkey(:active_low),"\0"); self; end
  def unexport; chkexp; @redis.del(rkey(:value),rkey(:direction),rkey(:edge),rkey(:active_low)); self; end
  ...
  def low?; chkexp; @triggered=false; @redis.getbit(rkey(:value),0)==0; end
  def high?; !low?; end
  def set_low(v=true); chkexp
    new = v ? 0 : 1
    old = @redis.setbit(rkey(:value),0,new)
    @redis.publish('gpio:edge',@id) if
  ...
  class << self
    def select(gpios, timeout:nil)
      redis = Redis.new
      begin
        selected = gpios.select{|g|g.instance_variable_get('@triggered')}
        return selected if selected != []
        Timeout.timeout(timeout){redis.subscribe('gpio:edge'){|s|s.message{|_,id|
          selected = gpios.find{|g|g.id==id.to_i}
          if selected
            selected.instance_variable_set('@triggered', true)
            redis.unsubscribe
          end
        }}}
        [selected]
